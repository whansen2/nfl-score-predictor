...
def load_prediction_context(weeks_to_include: int = 4) -> tuple[list[str], list[str]]:
    ...
    WEEK_ORDER = ["SuperBowl", "ConfChamp", "Division", "WildCard"] + [
        str(i) for i in range(18, 0, -1)
    ]
    df["Week"] = df["Week"].astype(str)
    df["WeekOrder"] = df["Week"].apply(
        lambda x: WEEK_ORDER.index(x) if x in WEEK_ORDER else 999
    )
    ...
    def create_summary(row):
        ...
        return (
            f"Week {row['Week']}: {row['Away Team']} ({row['Away Score']}) "
...
def get_prediction_stats() -> str:
    ...
    return (
        f"Model Summary: {total_games} games predicted, "
...
def get_flag_summary() -> str:
    ...
    close_calls = len(
        flagged[flagged["Upset Flag"].str.contains("Close Call", na=False)]
    )
    upsets = len(
        flagged[flagged["Upset Flag"].str.contains("Potential Upset", na=False)]
    )
...
def chat():
    ...
    typer.secho(
        "💬 Chat with Blitz about your NFL predictions! Type 'exit' to quit.\n",
    ...
    system_prompt = (
        "You are Blitz, an expert NFL analyst specializing in Linear Regression "
        "prediction model analysis. "
        "You analyze predictions made by a scikit-learn Linear Regression model "
        "that uses 6 key team statistics: "
        "conversion rates (Sc%_x, Sc%_y), first downs per game, yards per play, "
        "red zone efficiency, and turnover rates.\n\n"
        "ANALYSIS PRINCIPLES:\n"
        "- You ONLY discuss the prediction data provided below - never invent "
        "stats or external knowledge\n"
        "- Focus on spreads, totals, potential upsets, and close games\n"
        "- When discussing 'flags': Close Call = <4 point spread, Potential Upset "
        "= lower win team favored\n"
        "- Be conversational but analytical - explain what the predictions "
        "suggest\n"
        "- Use exact week labels from data (SuperBowl, ConfChamp, Division, "
        "WildCard, or numbers)\n\n"
...
def predict():
    ...
    if path.exists():
        ...
        typer.secho(
            "No prediction file found. Run `make run` from the root directory first.",
            fg=typer.colors.RED,
        )
    ...
    if path.exists():
        ...
        if not flagged.empty:
            ...
            for _, row in flagged.iterrows():
                ...
                typer.echo(
                    f"{flag_type} Week {row['Week']}: "
                    f"{row['Away Team']} @ {row['Home Team']}"
                )
                typer.echo(
                    f"   Predicted: {row['Away Score']}-{row['Home Score']} "
                    f"(Spread: {spread})"
                )
        ...
        typer.secho(
            "No flagged file found. Enable ENABLE_UPSETS_AGENT=true in .env "
            "and run `make run`.",
            fg=typer.colors.RED,
        )
...
def recap():
    ...
    if not flagged_path.exists():
        typer.secho(
            "No flagged prediction file found. Enable ENABLE_UPSETS_AGENT=true "
            "and run prediction model first.",
            fg=typer.colors.RED,
        )
        raise typer.Exit()
    ...
    # NFL week ordering
    WEEK_ORDER = [str(i) for i in range(1, 19)] + [
        "WildCard",
        "Division",
        "ConfChamp",
        "SuperBowl",
    ...
    week_input = input(
        "Which week to analyze? (e.g., '1', 'WildCard', 'SuperBowl' or "
    ...
    df_week = flagged_df[
        flagged_df["Week"].astype(str).str.lower() == str(week_input).lower()
    ]

    if df_week.empty:
        typer.secho(
            f"No predictions found for Week '{week_input}'", fg=typer.colors.YELLOW
        )
        raise typer.Exit()
    ...
    system_prompt = (
        ...
        "Analyze ONLY the prediction data provided. Focus on what the statistical "
        "model suggests about each game. "
        "Explain the significance of spreads, totals, and flags in practical "
        "terms for betting/fantasy context. "
    ...
    for _, row in df_week.iterrows():
        ...
        Context: This is a {game_type} prediction from a Linear Regression model
        using team statistics (conversion rates, yards per play, red zone
        ...
        typer.echo(f"\n{icon} {matchup}")
        typer.echo(
            f"   Prediction: {row['Away Score']}-{row['Home Score']} | "
